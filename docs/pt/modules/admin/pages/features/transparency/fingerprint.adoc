...
Its goal is to provide a way to give an informal "receipt" to a participant so they can detect tampering. Uma impressão digital é uma representação
de hash do conteúdo. É útil garantir que o conteúdo não tenha sido adulterado, como uma única modificação
resultaria em um valor totalmente diferente. It's calculated using a *SHA256 hashing algorithm*. A fim de replicá-lo
