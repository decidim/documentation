...
NOTE: Comments need to be enabled by administrators, otherwise participants are not able to comment. To learn how to enable comments on a component, please refer to the xref:features:components.adoc[Components] article.
...
[cols="10h,~,~,~"]
|===
|Tipo |Descripción |Syntax |Example
...
When clicking on a hashtag in a comment or another content, participants are redirected to the global search.
