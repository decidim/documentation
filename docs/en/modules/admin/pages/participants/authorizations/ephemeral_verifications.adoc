...
This functionality allows participants to participate without registering on the platform, simply by verifying their identity with the system chosen by the organisation. The key features are:
...
- **Link to an Existing Verification Method:** Ephemeral verification must be linked to a pre-existing verification method (e.g., an organizational census).
...
To enable Ephemeral Verification, follow these steps:

1. Access to the system panel.
2. Enable the ephemeral-verification method.
3. Link it to an existing verification method (e.g., organization census).
...
To better understand how this feature works, we explain the diferents flows and edge-cases as follows:
