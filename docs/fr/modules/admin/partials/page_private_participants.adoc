...
For instance, a space dedicated to the members of a commission in a big organization.
...
* The file must have two columns: emails and names of the participants you want to import.
* Le fichier ne doit pas avoir d'en-têtes. That means that you do not have to add a row that says "email, name"
* You must avoid using invalid chars like `<>?%&^*#@()[]=+:;"{}\|` in the name.
* The separator between columns must be a comma (",").

=== Invitation

NOTE: If the email is already linked to an account on the instance, the participant won't receive an invitation email, but will be added to the space.
...
* *Name*: Name of the private participant. If the account already exists, it is automatically changed to the account username.
* *Email*: Email of the participant.
* *Invitation sent at*: Day and hour when the invitation email was sent (if participant doesn't already have an account).
* *Invitation accepted at*: Day and hour when the invitation email was accepted by the participant.
...
|If the invitation wasn't accepted, send it again.
